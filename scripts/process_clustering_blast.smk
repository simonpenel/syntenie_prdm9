...
rule blast_db:
    ...
    shell:
        """
        makeblastdb  -in {input.concat} -out {output.blast_db}/concatenated_proteomes -dbtype prot -title concatenated_proteomes
        """ 
...
rule blast_compare:
    ...
    input:
        ...
        concat="concatenated_proteomes.faa"
    output:
        blast_out="concatenated_proteomes.blastout"
    shell:
        """
        blastp  --faster  -query {input.concat} -db {input.blast_db}/concatenated_proteomes -out {output} -evalue 1e-3  -outfmt 6 -num_threads 20
        """
